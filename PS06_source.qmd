...
* I believe there is a moderately strong correlation between `hs` and `income`, and that the relationship is linear, with acknowledgement of the outliers.
:::
...
* The intercept, meaning the value where `hs` is 0, is $-95647.43 for high `urbanization` areas and $-131041.39 for low `urbanization` areas, meaning tha high urbanization areas have the higher intercept.
:::
...
* Yes, the two slope lines appear to be very similar for the two levels of urbanization.
:::
...
* I believe that, because of the relative similarity of the two slopes, it is best if we use a parallel slopes model to easier suit our needs.
...
ggplot(hate_crimes, aes(x = urbanization, y = income))+
  geom_boxplot()+
  labs(x = "Level of urbanization in states", y = "Median income per state", title = "Relationship between average household income and level of urbanization per state")
```
:::
...
med_income_table <- med_income_model |> 
  get_regression_table() 
  kable(med_income_table)
...
ggplot(med_income_model, aes(x = hs, y = income, color = urbanization))+
  geom_point()+
  labs(x = "Median income per household in state", y = "percentage of adults with high-school degrees", 
...
* The two slopes, while having the same slope, do not share an intercept. They are parallel, and thus do not meet at any one point, including intercept.
:::
...
# Type your code and comments inside the code chunk
med_income_table |>
  summarise(intercept = med_income_table[1,2], slope = med_income_table[2,2])
```

* The intercept in the states of "high" levels of urbanization is $-113,725.19.

* The slope for the states of both high and low urbanization is $1,986.79 per unit of `hs`.
:::
...
* The intercept for states with "low" urbanization is -113725.193-7333.326 = $-121,058.52
:::
...
* The median household income is greater for states of high urbanization. Is is greater by $7,333.33 per unit of `hs`.
:::
...
predict(med_income_model, newdata = data)
```
...
predict(med_income_model, newdata = data1)
```
...
predict(med_income_model, newdata = data2)
```
...
predict(med_income_model, newdata = dataM)
```

* The observed income for Maine was $51,710.

* The prediction for Maine was $57,752.93.

* The residual for Maine is 57,752.93-51,710 = $6042.93.

* Our model overestimated the income for this state by $6,042.93
:::
...
* I would recommend protecting areas with high vegetation, as the slope appears to increase as vegetation increases.
:::
...
* 30=15.5+0.259*veg, so 55.99% vegetation
...
predict(voles_mod, newdata = dataV)
```

* I would predict approximately 35 voles for an area with these conditions.
:::
